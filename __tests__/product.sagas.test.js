import { onFetchProductsStart, onFetchProductTypesStart, onEditProductStart, onAddProductStart, onDeleteProductStart } from '../src/redux/product/product.sagas'
...
describe('testing fetching of products', () => {
  ...
  const sagaTester = new SagaTester({ initialState, reducers: { reducer: productReducer }})
  ...
  it('tests fetching products success', async () => {
    ...
    mockAxios.get.mockImplementationOnce(() => Promise.resolve({ data: ['prod1', 'prod2']}))
    sagaTester.dispatch({ type: productActionTypes.FETCH_PRODUCTS_START, payload: 'pizzas' })
    ...
    expect(mockAxios.get.mock.calls[0][0]).toBe(`${process.env.BACKEND_URL}/products/?type=pizza`)
    ...
    expect(sagaTester.numCalled(productActionTypes.FETCH_PRODUCTS_START)).toBe(1)
    expect(sagaTester.numCalled(productActionTypes.FETCH_PRODUCTS_SUCCESS)).toBe(1)
  })
  ...
  it('tests fetching products error', async () => {
    ...
    // Actions are called only once
    expect(sagaTester.numCalled(productActionTypes.FETCH_PRODUCTS_START)).toBe(1)
    expect(sagaTester.numCalled(productActionTypes.FETCH_PRODUCTS_FAILURE)).toBe(1)
  })
})
...
describe('testing fetching of product types', () => {  
  ...
  it('tests fetching product types success', async () => {
    ...
    mockAxios.get.mockImplementationOnce(() => Promise.resolve({ data: ['type1', 'type2']}))
    sagaTester.dispatch({ type: productActionTypes.FETCH_PRODUCT_TYPES_START, payload: '12345' })
    ...
    expect(mockAxios.get.mock.calls.length).toBe(1)
    expect(mockAxios.get.mock.calls[0][1]).toEqual({ headers: { 'Authorization': 'Bearer 12345' }})
    ...
    // Actions are called only once
    expect(sagaTester.numCalled(productActionTypes.FETCH_PRODUCT_TYPES_START)).toBe(1)
    expect(sagaTester.numCalled(productActionTypes.FETCH_PRODUCT_TYPES_SUCCESS)).toBe(1)
  })
  ...
  it('tests fetching product types failure', async () => {
    ...
    // Actions are called only once
    expect(sagaTester.numCalled(productActionTypes.FETCH_PRODUCT_TYPES_START)).toBe(1)
    expect(sagaTester.numCalled(productActionTypes.FETCH_PRODUCT_TYPES_FAILURE)).toBe(1)
  })
})
...
describe('testing editing product', () => {
  ...
  it('tests editing product success', async () => {
    ...
    sagaTester.dispatch({ type: productActionTypes.EDIT_PRODUCT_START, payload: { product: {}, product_id: '445', token: '12345' }})
    ...
    expect(mockAxios.patch.mock.calls[0][0]).toBe(`${process.env.BACKEND_URL}/products/445`)
    ...
    expect(sagaTester.numCalled(productActionTypes.EDIT_PRODUCT_SUCCESS)).toBe(1)
  })

  it('tests editing product falure', async () => {
    // The state is initialized properly
    expect(sagaTester.getState()).toEqual(initialState)
    mockAxios.patch.mockImplementationOnce(() => Promise.reject({ response: { data: 'This is some error message' }}))
    ...
    // Actions are called only once
    expect(sagaTester.numCalled(productActionTypes.EDIT_PRODUCT_START)).toBe(1)
    expect(sagaTester.numCalled(productActionTypes.EDIT_PRODUCT_FAILURE)).toBe(1)
  })
})
...
describe('testing adding product', () => {
  ...
  it('tests adding product success', async () => {
    ...
    sagaTester.dispatch({ type: productActionTypes.ADD_PRODUCT_START, payload: { product: {}, token: '12345' }})
...
describe('testing deleting a product', () => {
  ...
  it('tests deleting product success', async () => {
    ...
    sagaTester.dispatch({ type: productActionTypes.DELETE_PRODUCT_START, payload: { _id: '445', token: '12345' }})
    ...
    expect(mockAxios.delete.mock.calls[0][0]).toBe(`${process.env.BACKEND_URL}/products/445`)
    ...
    // Actions are called only once
    expect(sagaTester.numCalled(productActionTypes.DELETE_PRODUCT_START)).toBe(1)
    expect(sagaTester.numCalled(productActionTypes.DELETE_PRODUCT_SUCCESS)).toBe(1)
  })
  ...
  it('tests deleting a product failure', async () => {
    ...
    // Actions are called only once
    expect(sagaTester.numCalled(productActionTypes.DELETE_PRODUCT_START)).toBe(1)
    expect(sagaTester.numCalled(productActionTypes.DELETE_PRODUCT_FAILURE)).toBe(1)
  })
})