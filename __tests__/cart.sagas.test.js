import { onAddOrderStart, onFetchOrdersStart} from '../src/redux/cart/cart.sagas'
...
describe('testing making new order', () => {
  ...
  const sagaTester = new SagaTester({ initialState, reducers: { reducer: cartReducer }})
  ...
  it('tests making an order success', async () => {
    ...
    sagaTester.dispatch({ type: cartActionTypes.ADD_ORDER_START, payload: { order: { orderObject: {} }, token: '12345'} })
...
describe('testing fetching orders', () => {
  ...
  it('tests fetching orders success', async () => {
    ...
    mockAxios.get.mockImplementationOnce(() => Promise.resolve({ data: ['order1', 'order2'] }))
    sagaTester.dispatch({ type: cartActionTypes.FETCH_ORDERS_START, payload: '12345' })
