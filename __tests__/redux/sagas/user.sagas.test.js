...
import {
  ...
  onUserSignUpStart,
  onEditUserStart,
} from '../../../src/redux/user/user.sagas'
...
describe('testing logging in user', () => {
  ...
  it('tests user login success', async () => {
    ...
    mockAxios.post.mockImplementationOnce(() =>
      Promise.resolve({
        data: { user: { name: 'Bob' }, userType: 'regular', token: '12345' },
      })
    )
    sagaTester.dispatch({
      type: userActionTypes.USER_LOG_IN_START,
      payload: { email: 'email@email.com', password: '33pass66' },
    })
...
describe('testing user editing', () => {
  ...
  it('tests user editing success', async () => {
    ...
    mockAxios.patch.mockImplementationOnce(() =>
      Promise.resolve({ data: { name: 'Mr Bob' } })
    )
    sagaTester.dispatch({
      type: userActionTypes.EDIT_USER_START,
      payload: { request: { requestObj: {} }, token: '12345' },
    })
