...
export function* fetchProductsStartAsync(action) {
  const typeOfProducts = ['pizzas', 'drinks', 'desserts'].includes(
    action.payload
  )
    ? action.payload
    : 'all'
  ...
  try {
    const response = yield axios.get(
      `${process.env.BACKEND_URL}/products${
        reqQueryParam !== '' ? `/?type=${reqQueryParam}` : ''
      }`
    )
...
export function* fetchProductTypesAsync(action) {
  try {
    const response = yield axios.get(
      `${process.env.BACKEND_URL}/product-types`,
      {
        headers: {
...
export function* onFetchProductTypesStart() {
  yield takeLatest(
    productActionTypes.FETCH_PRODUCT_TYPES_START,
    fetchProductTypesAsync
  )
}
...
export function* editProductStartAsync(action) {
  try {
    ...
    productKeys.forEach((key) =>
      formData.append(key, action.payload.product[key])
    )
    ...
    yield axios.patch(
      `${process.env.BACKEND_URL}/products/${action.payload.product_id}`,
      formData,
      {
        headers: {
...
export function* addProductStartAsync(action) {
  try {
    ...
    yield axios.post(`${process.env.BACKEND_URL}/products`, formData, {
      headers: {
        Authorization: `Bearer ${action.payload.token}`,
        'Content-Type': 'multipart/form-data;boundary="boundary"',
      },
...
export function* deleteProductStartAsync(action) {
  try {
    yield axios.delete(
      `${process.env.BACKEND_URL}/products/${action.payload._id}`,
      {
        headers: {
          Authorization: `Bearer ${action.payload.token}`,
        },
